...
from downloader import SpotdlDownloader
from csv_manager import CSVManager
from utils import PlaylistReader, UserInput
from logger import Logger
from error_handler import ErrorHandler, ValidationError, SpotifyError
...
def process_playlist(
    ...
    try:
        ...
        Logger.info(f"Need to download {len(missing_tracks)} songs")
        
        # Download missing songs
        for idx, track in enumerate(missing_tracks, 1):
            Logger.progress(idx, len(missing_tracks), f"downloading")
...
def main():
    ...
    # Initialize clients
    Logger.header("Spotify Playlist Checker")
